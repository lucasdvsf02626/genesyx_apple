...
| 5 | What `profiles.theme` says for existing users | T20 made light the *local* default (`PreferencesRepository.swift:102`), but `apply(remote)` at `:196` overwrites it from the server on sign-in, so the server's value is the one that counts. **Reported 13 Aug: the column default is `dark`.** That is worse than the `system` case this row was written for — a row created by the default hands a brand-new user a preference she never expressed, on her first launch, and `migrateLegacySystemTheme()` (`:179`) deliberately leaves `dark` alone because from the client it is indistinguishable from a real choice. | SQL Editor: `select theme, count(*) from public.profiles group by theme;` — then apply `20260813_profiles_theme_default_light.sql` |
...
| P0-10 | **Privacy policy sign-off — release blocker** | ⬜ `PRIVACY_POLICY.md` claimed the app collects nothing, has no accounts and no servers, while it syncs six tables to Supabase and has three sign-in methods. `APP_STORE_SUBMISSION.md` §2 said the opposite in the same repo. Rewritten 13 Aug from the actual code and marked **DRAFT**. Brackets filled the same day from verifiable sources — controller `SF MEDIA & PR LTD` (the archive's signing identity), Supabase `eu-west-1`/Ireland (`supabase projects list`), Resend US (`send_partner_invite/index.ts:22` calls the global endpoint, not the EU one), EEA covered by UK adequacy so only the invite email needs the IDTA. **Still blocking:** (a) registered office address, deliberately not invented; (b) `hello@genesyx.co.uk` must actually be monitored — the policy now makes it the address for rights requests, which carry a one-month statutory deadline; (c) the children's age is written as 18 and the App Store age rating has not been set, so the two public documents are not yet reconciled; (d) a practitioner's read, health data being UK GDPR Article 9. Do not publish or submit until all four are done |
...
| P0-12 | Signed archive, version 1.2.0 (18) | ✅ 13 Aug — `build/Genesyx_1.2.0_18.xcarchive`, `Apple Distribution: SF MEDIA & PR LTD (M5L3MM75SG)`, `com.genesyx.app`, 1.2.0 (18), dSYM present. **Not uploaded.** P0-10 blocks submission, not archiving |
