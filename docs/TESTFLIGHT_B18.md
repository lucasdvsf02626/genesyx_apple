...
| P0-14 | **App Privacy declarations understate what is collected** | ⬜ Found 13 Aug by third-party audit, verified against the code. The App Privacy table in `APP_STORE_SUBMISSION.md` §2 and `PrivacyInfo.xcprivacy` do not declare **Name** (the display name, which the partner link actually shows to another person) or **Other User Content** (free-text notes in daily logs). Both are collected and both are linked to identity. Under-declaring is a common 5.1.1 rejection and, unlike the imaginary one in P0-10, this one is real. Fix both files together — App Review compares them |
| P0-16 | **The two clients erase different things from the same database** | ⬜ Found 13 Aug by cross-checking the Android repo, which shares this Supabase project. iOS deletes through the `delete_account` Edge Function; **Android 1.4.0 (`versionCode` 14) calls a `delete_current_user()` SQL RPC instead** (`SupabaseAuthService.kt:70`). They are not the same contract and each one misses what the other cleans. **iOS misses `user_supplements`** — Android ships it as a synced table (`UserSupplementsSection.kt:49`) and its `user_id` is `uuid NOT NULL` with **no foreign key** (`2026-07-29_user_supplements.sql:60`), so `auth.admin.deleteUser` cascades nothing and the rows survive forever, unreachable: RLS is `auth.uid() = user_id` and that uid is gone. The Edge Function's own list says "extend this list as the schema grows" (`index.ts:48`); the schema grew on 29 Jul and nobody did. **Android misses both email-keyed tables** — `partner_invites` addressed TO her via `invitee_email` and `waitlist_emails`, neither of which has an FK either, and its own schema notes the second (`schema.sql:107`). Also worth flagging: the Kotlin doc comment claims cascades do the work (`:64-65`) while the SQL comment claims the DB has none (`:90`); the schema actually has cascades on every table *except* the three above, so both comments mislead. **Checked 13 Aug: `user_supplements` does NOT exist on the live project** (`information_schema.tables` returned only `quiz_answers` and `waitlist_emails`), which inverts this entry. **The iOS half is latent, not live** — there are no rows to orphan, and adding the delete would have aborted `delete_account` on a missing relation and broken deletion for everyone. Leave the iOS list alone; revisit only if the table is ever created. **What the check found instead is an Android problem, and it may be severe.** (a) Android ships `user_supplements` as an "offline-first, synced" feature with a Room mirror (`UserSupplementsSection.kt:49`, `Migrations.kt:47`) against a server table that is not there — she adds supplements, sees them locally, they never land, and a new phone shows nothing. Same silent shape as the `food_groups` bug in pre-flight 3. (b) Worse: the 29 Jul `delete_current_user()` body *begins* with `delete from public.user_supplements`. plpgsql resolves table names at run time, not definition time, so if that version is the deployed one the whole function aborts and **nothing is deleted** — Android account deletion would be broken in production, not merely incomplete. The older `schema.sql:238` body has no such line and would work while still missing the email-keyed tables. ✅ **Read 13 Aug — the OLDER body is live.** `pg_get_functiondef` returned the `schema.sql:238` version: no `user_supplements` line, so the function does not abort and **Android account deletion works.** The severe case (b) is ruled out; case (a), the phantom synced table, still stands and belongs to the Android team. **But the same read found a worse thing, in the opposite place.** `pg_constraint` on `public.profiles` returns exactly two rows — `profiles_pkey` and `profiles_id_fkey (id → auth.users ON DELETE CASCADE)`. There is **no foreign key on `partner_id` at all.** `docs/supabase_schema.sql:21` declares `partner_id uuid references public.profiles (id) on delete set null`; **that constraint was never deployed.** So the mechanism Android silently relies on to unlink her partner does not exist: `delete_current_user()` never touches `partner_id`, nothing sets it null, and **his row keeps a pointer to a profile that no longer exists** — a stuck "connected" state he cannot clear from inside the app, because `profiles_update` only lets him write his own row and the pointer is the thing that is wrong. iOS is unaffected: `delete_account` nulls the partner's pointer explicitly as its first step (`index.ts:39-46`) rather than trusting a cascade. **Live blast radius today is zero** — `count(*) filter (where partner_id is not null)` is **0**, nobody is linked yet — which is exactly why this is cheap to fix now and expensive after the feature gets used. Two candidate fixes, both deferred pending the Section 4 partner work: add the missing FK, or add the explicit unlink to the RPC. Prefer the explicit unlink — it matches iOS and does not depend on a constraint that has already proven it can go missing |
