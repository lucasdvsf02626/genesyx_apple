...
**The binary maps to a known commit, which is the whole point of the tag.** The build 21 archive was cut from the tree at `93f5a13`, tagged **`v1.2.0-b21`**; that sentence describes a binary that is now history, because the §10 changes touch `App/` and `Tests/` and build 21 cannot be re-used.

**The build 22 tree is committed and tagged, so the archive that does not yet exist already has a commit to map to.** The §10 work is committed as **`fff30dc`** and tagged **`v1.2.0-b22`** (annotated: *"Genesyx 1.2.0 (22) — consent gate made honest at the surface + Profile fixes"*). Checked, not assumed: `git rev-parse v1.2.0-b22^{}` resolves to `fff30dc`, `git status --short` is empty, and `git diff v1.2.0-b22 -- App/ Sources/ Tests/ Packages/` returns nothing. Archive from this tree and the provenance sentence above can be written honestly about the binary that is actually uploaded — **cut the archive before touching the tree again**, because that property is only true until the next edit.
...
**Working tree reconciled, 18 Aug 2026.** Five stray uncommitted edits — to `LearnContent.swift`, `CycleContent.swift`, `PhCopy.swift`, `PhInsightLogicTests.swift` and `App_Inventory.md` — were reverted by hand. One of them had rewritten a plain-English Learn callout into dense clinical register; it passed the banned-phrase guards, which is worth knowing, because those guards catch unsafe claims and not unreadable ones. A sixth, a staged `brand_lockup/Contents.json`, was kept at the time and has since been reverted too — see below. None of the six was in build 21; the archive predates them all, so the binary was never affected.

**The sixth one was misread, and the correction is the point.** It was recorded here as "an Xcode asset reformat", which is what the diff looks like at a glance: every line changes because Xcode rewrites `"key":` as `"key" :`. Underneath the requoting it also **added four empty image slots** — 2x and 3x, light and dark — with no `filename` on any of them. That is a content change to an asset the app loads by string on three screens a reviewer meets first: `AuthView`, `ResetPasswordView` and `OnboardingFlowView`.

It was checked rather than assumed: the app was launched signed-out on the simulator and the lockup renders correctly, so the empty slots are inert and nothing was broken. It was reverted anyway, on 18 August, because it had no upside — both PNGs were untouched and the new slots are empty — while keeping it cost the one property the `v1.2.0-b21` tag exists to provide. `git diff v1.2.0-b21 -- App/ Sources/ Tests/` was empty at that point, which is what made the b21 archive an honest description of the tree. The tree has since moved to build 22; the equivalent check to run now is the one recorded against `v1.2.0-b22` above.

The lesson worth keeping: a whitespace-shaped diff can carry a content change, and "reformat" is a conclusion, not an observation. Read what the requoting is hiding before waving it through.
