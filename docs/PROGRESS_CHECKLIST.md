...
| ☐ | Link to Genesyx website science and Shettles content | **Blocked** | 14 Aug 2026 | Re-fetched live 14 Aug. Two slugs appeared today and **must not be wired**: `https://genesyx.co.uk/pages/vaginal-ph-fertility-science` is the same product/marketing copy as `/pages/ph-tracking` (H1 “Track your vaginal pH at home”, Google Play CTAs, **no citations**). `https://genesyx.co.uk/pages/shettles-method-evidence-limitations` is an empty page whose H1 is the slug — it does not call the method unproven and does not mention Wilcox. App grep of both paths: **0**. Unblocking still needs a **cited science page** and a **Shettles page framed as unproven**, then a small wiring change. **14 Aug, later:** both pages are now **drafted** at `docs/website/vaginal-ph-fertility-science.md` and `docs/website/shettles-method-evidence-limitations.md` — disclaimers top and bottom, every citation verified against a primary source (PMID or resolved DOI), Shettles stated as an unproven 1960s hypothesis. The drafts are **unpublished and unreviewed**, so the row stays **Blocked**: what remains is a named clinical reviewer, publication to Shopify, then the wiring change. The pH draft also **corrects the live page's healthy range from 3.8–5.0 to 3.8–4.5**, which is what the app enforces. |
...
| ✓ | Task | Status | Date | Notes |
|---|---|---|---|---|
| ☑ | Confirm current Add Partner behaviour | **Done — but WITHHELD from the public release** | 14 Aug 2026 | Invite, account acceptance, linking and unlinking all exist and work; a partner receives the display name only, health logs are owner-only, and no partner reminder feed exists. ⚠️ **Client decision D6, 14 Aug: partner linking is out of scope for the 1.2.0 public release.** `FeatureFlags.partnerInvites` is a compile-time `false`, so no Profile section and no invite deep link reaches it, and `AppContainer` no longer calls `partner.refresh()`. Nothing was deleted — Android shares this backend and does ship the feature. This row stays **Done** because the work is complete; it is being withheld, not descoped. See `CHANGE_LIST_PLAN.md` §0.2. |
...
| ☐ | Scope a privacy-controlled iPhone widget | **To do — deferred. NOT in 1.2.0** | 14 Aug 2026 | No WidgetKit target exists — `project.yml` declares three targets and none is an extension. Requires a separate privacy and data-refresh design. **Re-surveyed 14 Aug: nothing to remove or gate, because nothing was ever built, and no App Store copy mentions it.** `ReleaseScopeTests.testTheBuiltAppEmbedsNoAppExtensions` now walks the built `.app` so an extension cannot arrive unnoticed. |
| ☐ | Scope barcode scanning/meal photos | **To do — deferred. NOT in 1.2.0** | 14 Aug 2026 | No AVFoundation/VisionKit pipeline exists; treat as a future technical/product scope. **The build also carries no `NSCameraUsageDescription` or `NSPhotoLibraryUsageDescription`, so it cannot open a capture UI at all** — iOS terminates an app that tries without them. The only two mentions anywhere are code comments explaining why nutrient counting is not offered (`NutritionView.swift:519`, `NutritionContent.swift:37`). Guarded by `ReleaseScopeTests`. |
